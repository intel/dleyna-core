...
static void prv_monitor_keyfile(dleyna_settings_t *settings,
				const gchar *path)
{
	GFile *cur_file;
	...
	cur_file = g_file_new_for_path(path);
	monitor = g_file_monitor_file(cur_file, G_FILE_MONITOR_SEND_MOVED, NULL,
				      NULL);
			     ...
			     const gchar *loc_path)
{
	...
	if (settings->keyfile != NULL) {
		...
		prv_monitor_keyfile(settings, path);
	}
}
...
static void prv_keyfile_finalize(dleyna_settings_t *settings)
{
	...
	if (settings->monitor) {
		...
		settings->monitor = NULL;
		settings->handler_id = 0;
	}
...
static dleyna_settings_t *prv_old_dup(dleyna_settings_t *settings)
{
	dleyna_settings_t *old;

	old = g_new0(dleyna_settings_t, 1);

	old->never_quit = settings->never_quit;
	old->connector_name = settings->connector_name;
	old->log_type = settings->log_type;
	old->log_level = settings->log_level;
	old->netf_enabled = settings->netf_enabled;
	old->netf_entries = settings->netf_entries;
...
static void prv_reload(dleyna_settings_t *settings)
{
	...
	/* Save all setting variables only */
	old = prv_old_dup(settings);
	...
	if (settings->reload_cb)
		settings->reload_cb(settings, old);

	prv_keyfile_finalize(old);
